...
import useMemoTransactions from '@api/hooks/memos/useMemoTransactions.ts'
...
const excludedColumns = ['balance', 'check_number', 'fees', 'amount_credit']

const filteredColumns = columns.filter((column) => !excludedColumns.includes(column.prop))

const { data, error, isError, isFetching, isLoading } = useMemoTransactions(props.memoId)
