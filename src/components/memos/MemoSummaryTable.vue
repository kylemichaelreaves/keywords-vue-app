<template>
  <div>
    ...
    <el-card data-testid="memo-summary-card">
      ...
      <div v-if="summaryData" class="summary-stats" data-testid="memo-summary-stats">
        <el-statistic
          title="Total Amount Debit"
          :value="summaryData.sum_amount_debit"
          prefix="$"
          :precision="2"
          class="stat-item"
          :data-testid="`sum-amount-debit-${memoId}`"
          :data-value="summaryData.sum_amount_debit"
        />
        <el-statistic
          title="Transactions Count"
          :value="summaryData.transactions_count"
          class="stat-item"
          :data-testid="`transactions-count-${memoId}`"
          :data-value="summaryData.transactions_count"
        />
      </div>
...
const memoId = computed(() => parseInt(route.params.memoId as string))
...
const isLoadingCondition = computed(
  () => summaryLoading.value || summaryFetching.value || memoLoading.value || memoFetching.value,
)
</script>
