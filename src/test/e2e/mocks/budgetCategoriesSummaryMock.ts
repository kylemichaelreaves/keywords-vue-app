...
function generateParentCategory(
  categoryId: number,
  categoryName: string,
  sourceId: number,
...
function generateChildCategory(
  categoryId: number,
  childName: string,
  parentCategory: BudgetCategorySummary,
): BudgetCategorySummary {
  return {
    category_id: categoryId,
    ...
    budget_category: parentCategory.budget_category,
...
export function generateBudgetCategoryHierarchy(
  options: {
  ...
  const parentNames = faker.helpers
    .shuffle(Object.keys(categoryHierarchy) as CategoryHierarchyKey[])
  ...
  for (const parentName of parentNames) {
    ...
    if (includeChildren) {
      const childNames = categoryHierarchy[parentName]
