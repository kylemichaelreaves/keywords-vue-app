...
describe('getWeekRange', () => {
    it('should return the correct start and end dates for a given week string', () => {
        ...
        const expectedStartDate = DateTime.fromObject({ weekYear: 2024, weekNumber: 8}, {zone: 'UTC' }).startOf('week').toFormat('cccc MMMM dd');
        const expectedEndDate = DateTime.fromObject({ weekYear: 2024, weekNumber: 8}, {zone: 'UTC' }).endOf('week').toFormat('cccc MMMM dd');
    ...
    it('should handle the transition from one year to the next', () => {
        ...
        const expectedStartDate = DateTime.fromObject({ weekYear: 2023, weekNumber: 52}, {zone: 'UTC' }).startOf('week').toFormat('cccc MMMM dd');
        const expectedEndDate = DateTime.fromObject({ weekYear: 2023, weekNumber: 52}, {zone: 'UTC' }).endOf('week').toFormat('cccc MMMM dd');
